...
const CartPage = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      // Calculate cart totals for each plan type
          ...
          const dailyTotal = filteredResults.reduce((acc, product) => acc + product.dailyTotal, 0);
          const weeklyTotal = filteredResults.reduce((acc, product) => acc + product.weeklyTotal, 0);
          const monthlyTotal = filteredResults.reduce((acc, product) => acc + product.monthlyTotal, 0);

      // Find the highest total
      const highestTotal = Math.max(dailyTotal, weeklyTotal, monthlyTotal);

      setProductData(filteredResults);
      setCartTotal(highestTotal);
        }
      } catch (error) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity style={styles.lowerBar} onPress={() => console.log('Lower bar pressed')}>
