...
const ProductCard = ({ product, imageUri, quantity, quantity2, selectedDays, planType, total }) => {
  ...
  const validQuantity2 = typeof quantity2 === 'number' ? quantity2 : 0;

  if (validQuantity <= 0 && validQuantity2 <= 0) {
    return null;
  }
    ...
    <View style={{ justifyContent: 'center', alignItems: 'center' }}>
      <View style={styles.indiBox}>
        ...
        <View style={styles.orderDetails}>
          ...
          {validQuantity > 0 && <Text style={{ color: 'grey' }}>Quantity 1L: {validQuantity}</Text>}
          {validQuantity2 > 0 && <Text style={{ color: 'grey' }}>Quantity 500mL: {validQuantity2}</Text>}
...
const CartPage = () => {
          ...
          const promises = products.map(async (product) => {
            ...
            if (doc.exists) {
              ...
              return {
                name: product.name,
                imageUri: imageUrl,
                quantity: userData.quantity1L || userData.quantityBuffalo || userData.Gokulquantity1L || 0,
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity
        style={styles.lowerBar}
        onPress={() => {
          console.log('Lower bar pressed');
          navigation.navigate('SelectOption'); // Navigate to 'SelectOption'
        }}
      >
