...
const AmulGold = () => {
    ...
    try {
      ...
      if (currentUser) {
        ...
        if (quantity !== undefined && selectedDays !== undefined && selectedPlan !== undefined) {
          const orderRef = firebase.firestore().collection('ordersAmulGold').doc(currentUser.uid);
          const userRef= firebase.firestore().collection('users').doc(currentUser.uid);
          await userRef.update({
            goldquantity1L:quantity1L,
            goldquantity500mL: quantity,
            goldselectedDays: selectedDays,
            goldselectedPlan: selectedPlan,
          });
          await orderRef.set({
            name:userName,
            quantity500mL: quantity,
            quantity1L:quantity1L,
            totalGold:finalTotal,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const images = [
        'AmulGold2-removebg-preview.png',
        'AmulGoldBack.png',
        'AmulGoldEnergy.png',
      ...
      <ScrollView style={styles.ScrollView}>
        ...
        <View style={styles.MainQuantityHeader}>
          <View style={styles.QuantityHeaderContainer}>
            <Text style={styles.QuantityHeader}>Amul Gold X 500mL</Text>
          </View>
        ...
        <View style={styles.MainQuantityHeader}>
          <View style={styles.QuantityHeaderContainer}>
            <Text style={styles.QuantityHeader}>Amul Gold X 1000mL</Text>
          </View>
          <View style={styles.QuantityConatiner}>
            <View style={styles.priceContainerMain}>
